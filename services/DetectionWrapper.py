...
class DetectionWrapper:
    ...
    def _run_loop(self):
        try:
            ...
            self._event_engine.log(self.name, f"failed to start: {error}", level="ERROR")
            say(f"[{self.name}] failed to start: {error}")
            return
        ...
        while not self._stop_event.is_set():
            ...
            if frame is not None and frame_number != last_frame_number:
                ...
                try:
                    self.detector.process(frame, frame_number=frame_number)
                except Exception as error:
